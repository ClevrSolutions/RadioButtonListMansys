define([
    ...
    "dojo/_base/lang",
    "dojo/html",
    "dojo/text!RadioButtonListMansys/widget/template/RadioButtonList.html"
],
    function (declare, _WidgetBase, _TemplatedMixin, dom, dojoClass, dojoStyle, dojoConstruct, dojoAttr, dojoArray, lang, dojoHtml, widgetTemplate) {
        ...
        // Declare widget.
        return declare("RadioButtonListMansys.widget.AssocRadioButtonList", [_WidgetBase, _TemplatedMixin], {
            ...
            _setupWidget: function (callback) {
                ...
                if (this.get("disabled") || this.readonly) {
            ...
            _populateRadiobuttonOptions: function (objs, callback) {
                ...
                for (i = 0; i < objs.length; i++) {
                    mxObj = objs[i];
                    optionLabel = mxObj.get(this.RadioListItemAttribute);
                    label = (mxObj.get(this.defaultAttr)) ? optionLabel + " <i><strong>(" + this.defaultCaption + ")</strong></i>" : optionLabel;
                    freetext = (mxObj.get(this.freeTextEnabledAttr)) ? true : false
                    this._radioButtonOptions[mxObj.getGuid()] = {
                        obj: mxObj,
                        label: label,
                        freetext: freetext
                    }
            ...
            _createRadiobuttonNodes: function (callback) {
                ...
                nodelength = (this.inputNodes != null) ? this.inputNodes.children.length: 0;
                ...
                for (var option in this._radioButtonOptions) {
                    if (this._radioButtonOptions.hasOwnProperty(option)) {
                        ...
                        if (this._radioButtonOptions[option].freetext) {
                            freeTextValue = this._radioButtonOptions[option].obj.get(this.freeTextAttr);
                            freeTextNode = this._createFreeTextNode(option, this._radioButtonOptions[option], freeTextValue);
                            this._radioButtonOptions[option].freetextnode = freeTextNode;
                            dojoConstruct.place(freeTextNode, labelNode, "last");
                            this.connect(freeTextNode, "onkeyup", lang.hitch(this, this.eventOnChange, freeTextNode, this._radioButtonOptions[option].obj));
                        }
            ...
            _createRadiobuttonNode: function (key, option, index, container) {
                logger.debug(this.id + "._createRadiobuttonNode");
            ...
            _createFreeTextNode: function(key, option, value) {                
                var display = ("" + this._contextObj.get(this.entity) === key);
                var textarea = dojoConstruct.create("textarea", {
                    "rows": 2,
                    "class": "form-control mx-textarea-input mx-textarea-input-noresize",
                    "style": (display) ? "display: block" : "display: none"
                });
                dojoAttr.set(textarea, "innerHTML", value);
                
                if (this._isReadOnly ||
                    this._contextObj.isReadonlyAttr(this.entity)) {
                    dojoAttr.set(textarea, "readonly", "readonly");
                }
            ...
            _addOnclickToRadiobuttonItem: function (labelNode, rbvalue) {
                ...
                this.connect(labelNode, "onclick", lang.hitch(this, function () {
                    ...
                    if (this._radioButtonOptions[rbvalue].freetextnode) {
                        dojoStyle.set(this._radioButtonOptions[rbvalue].freetextnode, "display", "block"); 
                    }
                    var optionKeys = Object.keys(this._radioButtonOptions);
                    for (i = 0; i < optionKeys.length; i++) {
                        var key = optionKeys[i];
                        if (key != rbvalue && this._radioButtonOptions[key].freetextnode) {
                            dojoStyle.set(this._radioButtonOptions[key].freetextnode, "display", "none");
                        }
                    }
...
require(["RadioButtonListMansys/widget/AssocRadioButtonList"], function () {
    "use strict";
});
